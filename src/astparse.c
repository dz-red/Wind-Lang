...
int wind_parse(Token *toks, int count, Program *out,
    ...
    while (cur(&p) != TK_EOF) {
        ...
        /* use math — тянем весь модуль */
        if (cur(&p) == TK_KW_USE) {
            adv(&p);
            Token *m = expect(&p, TK_IDENT, "module name after 'use'");
            if (prog.nimports < 32) {
                prog.imports[prog.nimports].module = dup_s(m->text);
                prog.imports[prog.nimports].names  = NULL;
                prog.imports[prog.nimports].nnames = 0;
                prog.nimports++;
            }
            expect_eol(&p);
            skipnl(&p);
        ...
        /* take a, b from math — тянем только перечисленное */
        if (cur(&p) == TK_KW_TAKE) {
            adv(&p);
            ...
            do {
                Token *nm = expect(&p, TK_IDENT, "name after 'take'");
                if (n == cap) {
                    cap = cap ? cap * 2 : 4;
                    char **grown = (char **)realloc(names, (size_t)cap * sizeof *names);
                    if (!grown) perr(&p, "out of memory in take-list");
                    names = grown;
                }
                names[n++] = dup_s(nm->text);
            } while (accept(&p, TK_COMMA));

            expect(&p, TK_KW_FROM, "'from' after the list of names");
            Token *m = expect(&p, TK_IDENT, "module name after 'from'");

            if (prog.nimports < 32) {
                prog.imports[prog.nimports].module = dup_s(m->text);
                prog.imports[prog.nimports].names  = names;
                prog.imports[prog.nimports].nnames = n;
                prog.nimports++;
            }
            expect_eol(&p);
            skipnl(&p);
