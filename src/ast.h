 ...
 *     освобождаются в ast_free_*.
 *   - Каждый узел несёт line/col начала для будущих ошибок.
 *   - TokenKind переиспользуется для типов (KW_INT/FRAC/STR/...) и
 *     операторов (TK_PLUS, TK_EQ, ...), чтобы не плодить параллельный enum.
 */
